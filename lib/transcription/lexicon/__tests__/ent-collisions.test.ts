...
import { ENT_LEXICON } from "@/lib/transcription/lexicon/ent";
 ...
 * `entry()` in ent.ts drops auto-derived triggers below five characters; this
 * pins that, and pins that nothing left over fires INSIDE an unrelated word.
 *
 * WHY THIS CHECKS WORD BOUNDARIES AND THE OBSTETRICS TEST DOES NOT. ENT vocabulary overlaps head-and-neck surgery and the shared anatomy core by nature. A trigger
...
describe("ent lexicon triggers do not collide with the rest of the master lexicon", () => {
  it("no ent trigger fires inside an unrelated word elsewhere in the lexicon", () => {
    const others = MASTER_LEXICON.filter((e) => !(e.specialties ?? []).includes("ent"));
    const otherStrings = others.flatMap((e) => [e.term, ...(e.aliases ?? [])]).map((s) => s.toLowerCase());

    const collisions: string[] = [];
    for (const entry of ENT_LEXICON) {
