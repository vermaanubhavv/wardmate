...
import { DERMATOLOGY } from "@/lib/transcription/lexicon/dermatology";
 ...
 * `entry()` in dermatology.ts drops auto-derived triggers below five characters; this
 * pins that, and pins that nothing left over fires INSIDE an unrelated word.
 *
 * WHY THIS CHECKS WORD BOUNDARIES AND THE OBSTETRICS TEST DOES NOT. Skin vocabulary overlaps the medicine and oncology cores wherever drug reactions, infection and immunosuppression are concerned. A trigger
...
describe("dermatology lexicon triggers do not collide with the rest of the master lexicon", () => {
  it("no dermatology trigger fires inside an unrelated word elsewhere in the lexicon", () => {
    const others = MASTER_LEXICON.filter((e) => !(e.specialties ?? []).includes("dermatology"));
    const otherStrings = others.flatMap((e) => [e.term, ...(e.aliases ?? [])]).map((s) => s.toLowerCase());

    const collisions: string[] = [];
    for (const entry of DERMATOLOGY) {
