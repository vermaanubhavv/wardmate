...
import { EMERGENCY_MEDICINE } from "@/lib/transcription/lexicon/emergency-medicine";
 ...
 * emergency-medicine.ts drops auto-derived triggers below five characters; this pins that, and pins
 * that nothing left over fires INSIDE an unrelated word.
 *
 * Word boundaries rather than bare substrings, as the pulmonary test does it: an emergency department shares its vocabulary with every ward that admits from it. A real
...
describe("emergency-medicine lexicon triggers do not collide with the rest of the master lexicon", () => {
  it("no emergency trigger fires inside an unrelated word elsewhere in the lexicon", () => {
    const others = MASTER_LEXICON.filter((e) => !(e.specialties ?? []).includes("emergency-medicine"));
    const otherStrings = others.flatMap((e) => [e.term, ...(e.aliases ?? [])]).map((s) => s.toLowerCase());

    const collisions: string[] = [];
    for (const entry of EMERGENCY_MEDICINE) {
