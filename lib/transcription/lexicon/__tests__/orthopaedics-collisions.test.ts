...
import { ORTHOPAEDICS } from "@/lib/transcription/lexicon/orthopaedics";
 ...
 * orthopaedics.ts drops auto-derived triggers below five characters; this pins that, and pins
 * that nothing left over fires INSIDE an unrelated word.
 *
 * Word boundaries rather than bare substrings, as the pulmonary test does it: an orthopaedic ward shares its words with general surgery by nature — "wound", "fracture" and "drain" all sit inside longer legitimate phrases. A real
...
describe("orthopaedics lexicon triggers do not collide with the rest of the master lexicon", () => {
  it("no orthopaedic trigger fires inside an unrelated word elsewhere in the lexicon", () => {
    const others = MASTER_LEXICON.filter((e) => !(e.specialties ?? []).includes("orthopaedics"));
    const otherStrings = others.flatMap((e) => [e.term, ...(e.aliases ?? [])]).map((s) => s.toLowerCase());

    const collisions: string[] = [];
    for (const entry of ORTHOPAEDICS) {
