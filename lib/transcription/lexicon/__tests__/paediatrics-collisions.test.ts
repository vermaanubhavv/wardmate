...
import { PAEDIATRICS } from "@/lib/transcription/lexicon/paediatrics";
 ...
 * paediatrics.ts drops auto-derived triggers below five characters; this pins that, and pins
 * that nothing left over fires INSIDE an unrelated word.
 *
 * Word boundaries rather than bare substrings, as the pulmonary test does it: a children's ward names the same diseases as the adult wards, so the shared words are supposed to match. A real
...
describe("paediatrics lexicon triggers do not collide with the rest of the master lexicon", () => {
  it("no paediatric trigger fires inside an unrelated word elsewhere in the lexicon", () => {
    const others = MASTER_LEXICON.filter((e) => !(e.specialties ?? []).includes("paediatrics"));
    const otherStrings = others.flatMap((e) => [e.term, ...(e.aliases ?? [])]).map((s) => s.toLowerCase());

    const collisions: string[] = [];
    for (const entry of PAEDIATRICS) {
