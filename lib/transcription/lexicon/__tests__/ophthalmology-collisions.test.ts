...
import { OPHTHALMOLOGY } from "@/lib/transcription/lexicon/ophthalmology";
 ...
 * `entry()` in ophthalmology.ts drops auto-derived triggers below five characters; this
 * pins that, and pins that nothing left over fires INSIDE an unrelated word.
 *
 * WHY THIS CHECKS WORD BOUNDARIES AND THE OBSTETRICS TEST DOES NOT. Eye vocabulary overlaps the shared examination and imaging cores by nature. A trigger
...
describe("ophthalmology lexicon triggers do not collide with the rest of the master lexicon", () => {
  it("no ophthalmology trigger fires inside an unrelated word elsewhere in the lexicon", () => {
    const others = MASTER_LEXICON.filter((e) => !(e.specialties ?? []).includes("ophthalmology"));
    const otherStrings = others.flatMap((e) => [e.term, ...(e.aliases ?? [])]).map((s) => s.toLowerCase());

    const collisions: string[] = [];
    for (const entry of OPHTHALMOLOGY) {
