...
import { UROLOGY } from "@/lib/transcription/lexicon/urology";
 ...
 * urology.ts drops auto-derived triggers below five characters; this pins that, and pins
 * that nothing left over fires INSIDE an unrelated word.
 *
 * Word boundaries rather than bare substrings, as the pulmonary test does it: a urology ward shares "catheter", "stent" and "creatinine" with every other ward. A real
...
describe("urology lexicon triggers do not collide with the rest of the master lexicon", () => {
  it("no urology trigger fires inside an unrelated word elsewhere in the lexicon", () => {
    const others = MASTER_LEXICON.filter((e) => !(e.specialties ?? []).includes("urology"));
    const otherStrings = others.flatMap((e) => [e.term, ...(e.aliases ?? [])]).map((s) => s.toLowerCase());

    const collisions: string[] = [];
    for (const entry of UROLOGY) {
