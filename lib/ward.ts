...
export async function getActivePatients(wardId: string) {
  ...
  const { data: patients, error } = await supabase
    ...
    .select(
      "id, display_name, age_years, sex, bed, primary_diagnosis, admitted_on, surgery_date, post_op_day, admission_day, last_entry_at, template_family, template_variant, management"
    )
    .eq("ward_id", wardId)
    .eq("status", "active");
