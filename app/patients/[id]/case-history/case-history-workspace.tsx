...
/** A textarea with a hold-to-talk button that fills it from /api/transcribe. */
  ...
  async function start() {
    ...
    try {
      ...
      rec.onstop = async () => {
        ...
        form.append("audio", new Blob(chunksRef.current, { type: "audio/webm" }), "note.webm");
        try {
          const r = await fetch("/api/transcribe", { method: "POST", body: form });
          const data = await r.json();
          if (r.ok && data.text) onChange(value ? `${value.trim()} ${data.text}` : data.text);
        } catch {
